...
- (instancetype)initWithFrame:(CGRect)frame values:(NSArray *)valueArray titelsArr:(NSArray *)arr{
    self = [super initWithFrame:frame];
    if (self) {
        self.valueArray = valueArray;
        _LuckyTitelArray = arr;
        [self commonInit];
...
//        [descriptionLabelArray addObject:label];
        XMCircleTypeView *circleView = [[XMCircleTypeView alloc]initWithFrame:CGRectMake(10, 10, self.width-20, self.width-20)];
        circleView.backgroundColor = [UIColor clearColor];
        circleView.text = _LuckyTitelArray[i];
        NSLog(@"circleView.text====%@",circleView.text);
        circleView.textAttributes = @{NSFontAttributeName: [UIFont systemFontOfSize:8]};
        circleView.textAttributes = @{NSForegroundColorAttributeName: [UIColor whiteColor]};
        circleView.textAlignment = NSTextAlignmentCenter;
        circleView.baseAngle = (i-2) *36* M_PI /180;
        NSLog(@"baseAngle====%f",circleView.baseAngle);
        //    self.circleView.characterSpacing = 0.85;
        circleView.characterSpacing = 0;
...
- (UILabel *)descriptionLabelWithIndex:(NSInteger)index {
    ...
    NSString * aString =label.text;
    _attString = [[NSMutableAttributedString alloc] initWithString:aString];
    return label;
}
